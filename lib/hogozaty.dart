...
class _hogozatState extends State<hogozat> {
  ...
  Future getcontent()async {
    ...
    try {
      http.Response response = await http.get('http://aliensxtech.com/api/Bookingorderforservesreqyesters/3');
      if (response.statusCode == 200||response.statusCode == 201) {
        setState(() {
      f= json.decode(response.body);
        });
        content=json.decode(response.body)[1];
        for(int i=0;i<json.decode(response.body)[0].length;i++){
          content.add(json.decode(response.body)[0][i]);
        }
        print(f);
  ...
  Widget build(BuildContext context) {
    ...
    Navigator.pop(context);},child: Icon(Icons.arrow_back,color: Colors.black,)),
            ...
            if(Loading==false)Expanded(child: ListView.builder(itemCount: content.length,physics: BouncingScrollPhysics(),itemBuilder: (context, index)
              ...
              return Padding(
                    ...
                    child: Container(child: Padding(
                      ...
                      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,crossAxisAlignment: CrossAxisAlignment.start,children: [
                        ...
                        Padding(
                          ...
                          child: Column(mainAxisAlignment: MainAxisAlignment.start,crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              ...
                              ,  Text(content[index]['State']=='orderd'?"قيد الانتظار":"تم القبول",style: TextStyle(color: Colors.green,fontSize:  h*0.0132275*1.6,fontWeight: FontWeight.bold,fontFamily: 'Cairo',))
                        ...
                        Padding(
                          padding: const EdgeInsets.all(8.0),
                          child: Text(moment.from(DateTime.tryParse(content[index]['created_at'])),style: TextStyle(fontSize: 10),),
                        ),
                      ],),
                      ...
                      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,crossAxisAlignment: CrossAxisAlignment.start,children: [
                        Padding(
                          ...
                          child: Column(mainAxisAlignment: MainAxisAlignment.start,crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text('نوع الحجز', style: TextStyle(color: Colors.grey,fontSize:  h*0.0132275*1.4,fontWeight: FontWeight.bold,fontFamily: 'Cairo',))
                              ,  Text(content[index]['Booking_kind'].toString(), style: TextStyle(color: Colors.black,fontSize:  h*0.0132275*1.6,fontWeight: FontWeight.bold,fontFamily: 'Cairo',))
                              ,SizedBox(height: 10,),
                              ...
                              ,  Text(content[index]['max_budget'] .toString(), style: TextStyle(color: Colors.black,fontSize:  h*0.0132275*1.6,fontWeight: FontWeight.bold,fontFamily: 'Cairo',))
                              ,SizedBox(height: h*0.0132275/2,),
                            ],
                          ),
                        ),
